...
type StageMyHomePhoto struct {
	FileName string `json:"fileName"`
	Mime     string `json:"mime"`
	Base64   string `json:"base64"`
}

type StageMyHomeInboundRequest struct {
...
type StageMyHomeInboundResponse struct {
	NextMessage string `json:"nextMessage"`
}

type StageMyHomeTextRequest struct {
	AgentNumber string `json:"agentNumber"`
	Text        string `json:"text"`
}

type StageMyHomeTextResponse struct {
...
func stageMyHomeBaseURL() string {
	if value := os.Getenv("STAGEMYHOME_URL"); value != "" {
		return strings.TrimRight(value, "/")
	}
	return "http://localhost:5173"
}

func normalizePhoneForStageMyHome(value string) string {
...
func sendTextToStageMyHome(client *whatsmeow.Client, sender string, content string) {
	...
	if err != nil {
		fmt.Printf("StageMyHome text bridge post failed: %v\n", err)
		return
	}
	defer resp.Body.Close()
	var payload StageMyHomeTextResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		fmt.Printf("StageMyHome text bridge decode failed: %v\n", err)
		return
	}
	if payload.Reply != "" {
...
func sendPhotoToStageMyHome(client *whatsmeow.Client, sender string, localPath string, mediaType string, filename string) {
	...
	req := StageMyHomeInboundRequest{
		...
		Photos: []StageMyHomePhoto{{
			...
			Base64:   binaryBase64(bytes),
		}},
	}

	body, _ := json.Marshal(req)
	resp, err := http.Post(stageMyHomeBaseURL()+"/api/whatsapp/inbound", "application/json", bytesReader(body))
	if err != nil {
		fmt.Printf("StageMyHome bridge post failed: %v\n", err)
		return
	}
	defer resp.Body.Close()
	var payload StageMyHomeInboundResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		fmt.Printf("StageMyHome bridge decode failed: %v\n", err)
		return
	}
...
func binaryBase64(data []byte) string {
	...
	for i := 0; i < len(data); i += 3 {
		...
		out.WriteByte(alphabet[(val>>18)&63])
		out.WriteByte(alphabet[(val>>12)&63])
		if n > 1 {
			out.WriteByte(alphabet[(val>>6)&63])
		} else {
			out.WriteByte('=')
		}
		if n > 2 {
			out.WriteByte(alphabet[val&63])
		} else {
			out.WriteByte('=')
		}
	}
